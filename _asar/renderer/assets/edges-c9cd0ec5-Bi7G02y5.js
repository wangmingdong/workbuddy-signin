...
import { B as utils, C as init_mermaid_59c9be08, E as log$1, J as init_src, M as sanitizeText$2, St as select_default, Y as basis_default, Z as line_default, _ as getConfig, p as evaluate, u as decodeEntities } from "./mermaid-59c9be08-DUdjYWP7.js";
import { n as createText, r as init_createText_31840e8c } from "./createText-31840e8c-BTw4yiRB.js";
...
var insertMarkers, extension, composition, aggregation, dependency, lollipop, point, circle$1, cross, barb, markers, insertMarkers$1, createLabel, createLabel$1, labelHelper, updateNodeBounds, intersectRect, intersectRect$1, intersect, note, note$1, expandAndDeduplicateDirections, getArrowPoints, formatClass, getClassesFromNode, question, choice, hexagon, block_arrow, rect_left_inv_arrow, lean_right, lean_left, trapezoid, inv_trapezoid, rect_right_inv_arrow, cylinder, rect, composite, labelRect, rectWithTitle, stadium, circle, doublecircle, subroutine, start, forkJoin, end, class_box, shapes, nodeElems, insertNode, setNodeElem, clear$1, positionNode, getSubGraphTitleMargins, markerOffsets, pointTransformer, getLineFunctionsWithOffset, addEdgeMarkers, arrowTypesMap, addEdgeMarker, edgeLabels, terminalLabels, clear, insertEdgeLabel, positionEdgeLabel, outsideNode, intersection, cutPathAtIntersect, insertEdge;
var init_edges_c9cd0ec5 = __esmMin((() => {
	init_mermaid_59c9be08();
	init_src();
	init_createText_31840e8c();
	insertMarkers = (elem, markerArray, type, id) => {
		markerArray.forEach((markerName) => {
			markers[markerName](elem, type, id);
		});
	};
	extension = (elem, type, id) => {
		log$1.trace("Making markers for ", id);
		elem.append("defs").append("marker").attr("id", id + "_" + type + "-extensionStart").attr("class", "marker extension " + type).attr("refX", 18).attr("refY", 7).attr("markerWidth", 190).attr("markerHeight", 240).attr("orient", "auto").append("path").attr("d", "M 1,7 L18,13 V 1 Z");
		elem.append("defs").append("marker").attr("id", id + "_" + type + "-extensionEnd").attr("class", "marker extension " + type).attr("refX", 1).attr("refY", 7).attr("markerWidth", 20).attr("markerHeight", 28).attr("orient", "auto").append("path").attr("d", "M 1,1 V 13 L18,7 Z");
	};
	composition = (elem, type, id) => {
		elem.append("defs").append("marker").attr("id", id + "_" + type + "-compositionStart").attr("class", "marker composition " + type).attr("refX", 18).attr("refY", 7).attr("markerWidth", 190).attr("markerHeight", 240).attr("orient", "auto").append("path").attr("d", "M 18,7 L9,13 L1,7 L9,1 Z");
		elem.append("defs").append("marker").attr("id", id + "_" + type + "-compositionEnd").attr("class", "marker composition " + type).attr("refX", 1).attr("refY", 7).attr("markerWidth", 20).attr("markerHeight", 28).attr("orient", "auto").append("path").attr("d", "M 18,7 L9,13 L1,7 L9,1 Z");
	};
	aggregation = (elem, type, id) => {
		elem.append("defs").append("marker").attr("id", id + "_" + type + "-aggregationStart").attr("class", "marker aggregation " + type).attr("refX", 18).attr("refY", 7).attr("markerWidth", 190).attr("markerHeight", 240).attr("orient", "auto").append("path").attr("d", "M 18,7 L9,13 L1,7 L9,1 Z");
		elem.append("defs").append("marker").attr("id", id + "_" + type + "-aggregationEnd").attr("class", "marker aggregation " + type).attr("refX", 1).attr("refY", 7).attr("markerWidth", 20).attr("markerHeight", 28).attr("orient", "auto").append("path").attr("d", "M 18,7 L9,13 L1,7 L9,1 Z");
	};
	dependency = (elem, type, id) => {
		elem.append("defs").append("marker").attr("id", id + "_" + type + "-dependencyStart").attr("class", "marker dependency " + type).attr("refX", 6).attr("refY", 7).attr("markerWidth", 190).attr("markerHeight", 240).attr("orient", "auto").append("path").attr("d", "M 5,7 L9,13 L1,7 L9,1 Z");
		elem.append("defs").append("marker").attr("id", id + "_" + type + "-dependencyEnd").attr("class", "marker dependency " + type).attr("refX", 13).attr("refY", 7).attr("markerWidth", 20).attr("markerHeight", 28).attr("orient", "auto").append("path").attr("d", "M 18,7 L9,13 L14,7 L9,1 Z");
	};
	lollipop = (elem, type, id) => {
	...
	insertMarkers$1 = insertMarkers;
	createLabel = (_vertexText, style, isTitle, isNode) => {
	...
	createLabel$1 = createLabel;
	labelHelper = async (parent, node, _classes, isNode) => {
		let classes;
	...
	note$1 = note;
	expandAndDeduplicateDirections = (directions) => {
	...
	getArrowPoints = (duplicatedDirections, bbox, node) => {
	...
	rect_left_inv_arrow = async (parent, node) => {
		const { shapeSvg, bbox } = await labelHelper(parent, node, getClassesFromNode(node, void 0), true);
	...
	inv_trapezoid = async (parent, node) => {
		const { shapeSvg, bbox } = await labelHelper(parent, node, getClassesFromNode(node, void 0), true);
		const w = bbox.width + node.padding;
		const h = bbox.height + node.padding;
		const points = [
	...
	rect_right_inv_arrow = async (parent, node) => {
		const { shapeSvg, bbox } = await labelHelper(parent, node, getClassesFromNode(node, void 0), true);
		const w = bbox.width + node.padding;
		const h = bbox.height + node.padding;
		const points = [
	...
	doublecircle = async (parent, node) => {
		const { shapeSvg, bbox, halfPadding } = await labelHelper(parent, node, getClassesFromNode(node, void 0), true);
		const gap = 5;
	...
	nodeElems = {};
	insertNode = async (elem, node, dir) => {
		let newEl;
		let el;
		if (node.link) {
			let target;
	...
	getSubGraphTitleMargins = ({ flowchart }) => {
		var _a, _b;
	...
	addEdgeMarker = (svgPath, position, arrowType, url, id, diagramType) => {
	...
	intersection = (node, outsidePoint, insidePoint) => {
  ...
  node        : x:${node.x} y:${node.y} w:${node.width} h:${node.height}`);
	...
	insertEdge = function(elem, e, edge, clusterDb, diagramType, graph, id) {
...
//#endregion
export { updateNodeBounds as _, getLineFunctionsWithOffset as a, insertEdge as c, insertNode as d, intersectRect$1 as f, setNodeElem as g, positionNode as h, createLabel$1 as i, insertEdgeLabel as l, positionEdgeLabel as m, clear as n, getSubGraphTitleMargins as o, labelHelper as p, clear$1 as r, init_edges_c9cd0ec5 as s, addEdgeMarkers as t, insertMarkers$1 as u };
